...
class UpdateInstructionReadView(APIView):
    authentication_classes = [TokenAuthentication]
    permission_classes = [IsAuthenticated]

    def post(self, request, *args, **kwargs):
        # Access the user from the request (due to token authentication)
        user = request.user
        print(user)
        # Ensure the user has a profile and update instruction_read to True
        if hasattr(user, 'profile'):
            user.profile.instruction_agreed = True
            user.profile.save()

            return Response({"detail": "Instruction read status updated."}, status=status.HTTP_200_OK)
        else:
...
class LoginView(ObtainAuthToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.get("valid"):
            ...
            return Response({
                ...
                'jobs': jobs_count,
                'instruction_read': user.profile.instruction_agreed,
                'message': 'Success'
    ...
    def authenticate_user(self, username, password):
        ...
        try:
            ...
            if user and user.profile and not user.profile.profile_image:
                user.profile.delete()
                user.delete()
                return None
...
def send_application_email(email, username=""):
    ...
    try:
        ...
        send_mail(
            ...
            '',  # Plain text message (empty if you only use HTML content)
